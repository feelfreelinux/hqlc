#!/usr/bin/env python3
"""Quality benchmark for the HQLC codec across bitrates.

Roundtrips each clip in a directory through the hqlc binary at several
bitrates and reports waveform + transient metrics, optionally with
perceptual ViSQOL / Zimtohrli MOS. It does not build anything - point
--bin at any binary to compare builds with the same harness.

Metrics:
    SNR   time-domain SNR after alignment. Cheap sanity check; reads low
          on transients (castanets ~6-9 dB is normal, not a bug).
    P5    5th-percentile segmental SNR (2.7 ms windows). Catches localized
          error bursts (pre-echo, NF overshoot) that the mean hides.
    PE    pre-echo: codec error vs signal energy in the 20..2 ms before each
          attack. >0 dB means the error is louder than the signal there.
          "-" if the clip has no detected attacks.
    ViSQOL / Zim   perceptual MOS. Use >=20 ms windows, so nearly blind to
          pre-echo - judge transients on P5/PE, steady state on these.

Dependencies:
    SNR/P5/PE   numpy, scipy, ffmpeg (any Python 3.9+).
    --visqol    a docker image, built once:
                    docker build -f scripts/Dockerfile.visqol -t visqol-local .
                (without it, ViSQOL reports 0.000)
    --zim       the prebuilt mos/zimtohrli _pyohrli.so is linked against one
                Python (currently 3.14); run with that interpreter or it
                segfaults. The script checks and tells you which one.

Usage:
    python3 scripts/snr_bench.py                        # SNR/P5/PE @ 96k/128k/196k
    python3 scripts/snr_bench.py --bitrates 96000,128000 -o out.csv
    python3.14 scripts/snr_bench.py --visqol --zim      # add perceptual metrics
    python3 scripts/snr_bench.py --bin build/hqlc --clips-dir test-clips2
"""
...
SEG_WIN = 128  # segmental window, ~2.7 ms at 48 kHz; shared by P5 and attack detection
SEG_SNR_CLAMP = (-20.0, 60.0)  # per-window SNR clamp (standard segSNR practice)
SEG_SILENCE_MS = 1e-6  # P5 skips reference windows below this (-60 dBFS)

# Attack = window jumping PRE_ATTACK_DB above the loudest of the preceding
# PRE_LOOKBACK windows, and above an absolute floor.
PRE_ATTACK_DB = 12.0
PRE_LOOKBACK = 8  # windows (~21 ms)
PRE_FLOOR_MS = 1e-4  # -40 dBFS: don't call noise-floor wiggle an attack
# Pre-echo region before an attack: -20 ms (max MDCT smear) to -2 ms (edge of
# acoustic pre-masking, closer than which the attack hides the error).
...
def load_aligned(ref_path: Path, deg_path: Path) -> tuple[np.ndarray, np.ndarray, int]:
    """Load both files and align on the codec's ~512-sample MDCT delay.
...
def seg_snr_p5(ref: np.ndarray, deg: np.ndarray) -> float:
    ...
    re_ = _window_energy(ref)
    ee = _window_energy(ref - deg)
    if len(re_) == 0:
        return 0.0
    keep = re_ > SEG_WIN * ref.shape[1] * SEG_SILENCE_MS
    if not keep.any():
        return 0.0
    snr = 10.0 * np.log10(re_[keep] / np.maximum(ee[keep], 1e-12))
...
def pre_echo(ref: np.ndarray, deg: np.ndarray, sr: int):
    ...
    Detects attacks, then measures codec error vs reference energy in the
    [-20 ms, -2 ms] window before each onset. Returns (worst_db, mean_db,
    n_attacks), or (None, None, 0) if no attacks.
    """
    err = ref - deg
    ...
    while w < len(db):
        if db[w] > floor_db and db[w] >= db[w - PRE_LOOKBACK:w].max() + PRE_ATTACK_DB:
            ...
            if lo >= 0:
                ...
                # Floor ref energy (~-100 dBFS) so silence before an attack
                # caps the ratio instead of dividing by zero.
...
def process_clip(args: tuple) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ref = tmpdir / "ref.wav"
        deg = tmpdir / "deg.wav"
        # Canonicalize to 48k s16 stereo so metrics are comparable across clips.
        subprocess.run(
...
def _check_zim_python() -> None:
    ...
    _pyohrli.so is a CPython extension linked against one specific Python;
    loading it elsewhere import-succeeds but segfaults on first call (surfacing
    only as an opaque BrokenProcessPool), so check the linked version up front.
    """
...
def main():
    ...
    # Per-bitrate summary: mean SNR/P5, worst P5, worst PE across clips.
